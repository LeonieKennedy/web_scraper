...
for i in range(0, 49):
    ...
    message_no = "2"
    print(i)
    url = "https://fakeinfo.net/fake-iphone-hangouts-message-generator"
    driver.get(url)
    ...
    for j in range(1, 4):
        ...
        time.sleep(5)
        # options > div:nth-child(2) > ul > li:nth-child(1) > a
        # options > div:nth-child(2) > ul > li:nth-child(2) > a
        WebDriverWait(driver, 40).until(EC.visibility_of_element_located((By.CSS_SELECTOR, ("#profile"+sender+"_message")))).clear()
        driver.find_element(By.CSS_SELECTOR, ("#profile"+sender+"_message")).send_keys(sentences[random.randint(0, 786)])

        if sender == "1":
            driver.find_element(By.CSS_SELECTOR, ("#tab-person1 > div > div:nth-child(4) > button")).click()
        else:
            driver.find_element(By.CSS_SELECTOR, ("#tab-person2 > div > div:nth-child(3) > button")).click()
